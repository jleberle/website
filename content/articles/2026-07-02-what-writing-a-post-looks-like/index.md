...
One worthwhile addendum, a day later having worked more fully with the system, I revised the process a tiny bit. Now when I'm finished with a post I run `site publish` which is my fish function that corresponds to my `publish-draft.sh` script. Running `site publish` without any options provides me a list of all the drafts currently in Obsidian and I can select one for publication.

In addition `publish-draft.sh` now copies over images used in the draft, runs them through the site's `to-avif` conversion script and rewrites the image embed to use the site's image short code rather than Obsidian's bare markdown link. 

By default the script will simply copy and convert, but adding the `--push` flag will also commit the post, and run `ship` which updates the writing log, commits everything, and runs the validation tests before pushing it to Github. 
